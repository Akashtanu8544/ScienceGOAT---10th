...
import { Award, Flame, CheckCircle, ArrowLeft, FlaskConical, Zap, Dna } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className={`p-3.5 rounded-2xl border text-center space-y-1 shadow-sm ${
          isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
        }`}>
          <div className="w-8 h-8 rounded-full bg-amber-500/10 text-amber-500 mx-auto flex items-center justify-center">
            <Flame className="w-4 h-4 fill-amber-500" />
          </div>
          <div className="text-sm font-black text-amber-500">{progress.streakDays}d</div>
          <div className="text-[10px] font-bold text-slate-500">स्ट्राइक</div>
        </div>

        <div className={`p-3.5 rounded-2xl border text-center space-y-1 shadow-sm ${
          isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
        }`}>
          <div className="w-8 h-8 rounded-full bg-blue-500/10 text-blue-500 mx-auto flex items-center justify-center">
            <Award className="w-4 h-4" />
          </div>
          <div className="text-sm font-black text-blue-500">{progress.totalPoints}</div>
          <div className="text-[10px] font-bold text-slate-500">ज्ञान अंक</div>
        </div>

        <div className={`p-3.5 rounded-2xl border text-center space-y-1 shadow-sm ${
          isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
        }`}>
          <div className="w-8 h-8 rounded-full bg-emerald-500/10 text-emerald-500 mx-auto flex items-center justify-center">
            <CheckCircle className="w-4 h-4" />
          </div>
          <div className="text-sm font-black text-emerald-500">{progressPercent}%</div>
          <div className="text-[10px] font-bold text-slate-500">पाठ्यक्रम</div>
        </div>
      </div>
